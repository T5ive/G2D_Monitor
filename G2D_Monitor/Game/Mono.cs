...
namespace G2D_Monitor.Game
{
    internal sealed class Mono : IDisposable
    {
        ...
        {
            this.hProc = hProc;
            allocAddr = VirtualAllocEx(hProc, IntPtr.Zero, new IntPtr(1096), MEM_COMMIT, EXECUTE_READ_WRITE); // Memory alloc returns error 5 (Access Denied) even when launched with admin right so assistant tool not works anymore, EAC block it or something, idk 
